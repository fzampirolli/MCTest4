            ...
            for t in provas:
                if fff is t[0]: # se prova é da mesma turma, acrescente
                    ...
                    if numQuestoes>0:
                        ...
                        print ">>>>",numQuestoes,maxQuestQuadro,numQuadros,numResto,maxQuadrosHoz,numQuadrosHoz,numQuestoesQuadro
                        ...
                        if numQuestoes < contadorQuestoes:
                           fimQuadro_ij[QL-1][QC-1] = numQuestoesQuadro*(numQuadros-1) + numResto

                        print ">>>>",numQuestoes,maxQuestQuadro,contadorQuestoes
                    ...
                    if int(MCTest_sheets)!=1: # foi escolhido a opção de gerar somente o quadro de respostas
                        ...
                        arqprova.write("\\begin{center}  \\begin{tabular}{l} \n")
                        
                        print fimQuadro_ij

                        numQuestStart = 1
                        
                        arqprova.write("\\vspace{0mm}")
                        for i in range(QL): # para cada linha de quadros
                            ...
                            for j in range(QC): # para cada coluna de quadros
                                ...
                                print "quadro",i,j, numQuestStart, numQuestEnd, numQuadros

                                arqprova.write("\\hspace{2mm}\\begin{tikzpicture}[font=\\tiny]\n")
                                arqprova.write("  \\foreach \\numLab in {%s,...,%s} {\n" % (numQuestStart, numQuestEnd) )
                                arqprova.write("    \\node[inner sep=3pt] at ({(\\numLab-%s+1) * 0.5},0) {\\numLab};\n" % (numQuestStart))
                                arqprova.write("  }\\vspace{-1mm}\n")
                                
                                arqprova.write("  \\foreach \\letter/\\line in {%s} {\n" % strResps)
                                arqprova.write("     \\begin{scope}[xshift=0cm,yshift=-(\\line)*5mm]\n")
                                arqprova.write("       \\node at (0.1,0) {\\letter};\n")
                                
                                arqprova.write("       \\foreach \\letter/\\position in {%s,...,%s} {\n" % (1, (numQuestEnd - numQuestStart+1)) )
                                ...
                                arqprova.write("\\end{tikzpicture}\\hspace{%s mm}\n" % 0)#(17-numQuadros))
                        ...
                        arqprova.write("\\end{tabular}\\end{center}\n")
